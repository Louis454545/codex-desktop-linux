...
- `scripts/lib/patch-chrome-plugin.js`
  Linux compatibility patcher for the upstream bundled Chrome plugin scripts. Adds Linux manifest checks, Brave native-host manifest coverage, Linux Chrome/Brave profile fallback, and Brave-aware diagnostics when upstream has not already shipped them.
